...
class ServiceInitTestCase(TestCase):
    def test_ServiceWillGetUserLink(self):
        ...
        versions_list = [ { VERSION : {'href' : '/'.join([HTTP_BASE,
                            API_BASE, VERSION])}, }, ]
        EDACloudService.make_request = mocksignature(EDACloudService.\
            make_request)
        responses = [ 
            { 'links' :
                { USER : {'href' : '/'.join(
                        [HTTP_BASE, API_BASE, VERSION, USER])} },
                },
        ...
        service.make_request.mock.\
            assert_called_with(service, 'GET', '/'.join(
                [HTTP_BASE, API_BASE, VERSION]), '')
        self.assertEqual('/'.join(
            [HTTP_BASE, API_BASE, VERSION, USER]), service.user_url)
